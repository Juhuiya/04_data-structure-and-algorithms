...
public class F_KruskalAlgorithm {
    ...
    public static Long solution(String input) throws IOException {
        ...
        BufferedReader br = new BufferedReader(new StringReader(input)); // 입력값 들고옴
        StringTokenizer st = new StringTokenizer(br.readLine());
        int V = Integer.parseInt(st.nextToken()); // 노드
        int E = Integer.parseInt(st.nextToken()); // 간선

        Edge[] edges = new Edge[E]; // 엣지가 필요하니까 엣지 객체 만들고
        for (int i = 0; i < E; i++) {
        ...
        // 2. 가중치가 작은 간선부터 선택해나가는 작업 시작
        for(Edge e : edges) {

            // 각각의 정점이 연결되어 있는지 확인
            if(find(e.u) != find(e.v)) {
                // 연결이 되어있지 않다면 정점을 연결함
                union(e.u, e.v);
                totalWeight += e.weight;
            }
        }

        return totalWeight; // 가장 적은 내용이 어디있는지 확인가능
    }
}
