...
public class D_DijkstraAlgorithm {
    ...
    static class Edge implements Comparable<Edge> {
        ...
        public int compareTo(Edge o) {
            return this.cost - o.cost; // 가중치의 오름차순의 우선순위
        }
    }
    ...
    public static String solution(String input) throws IOException {
        ...
        Arrays.fill(dis, Integer.MAX_VALUE); //fill 메소드는 채우는 메소드
        ...
        while (!pq.isEmpty()) {
            ...
            /* 기준 정점과 연결된 이웃 정점을 큐에 추가하는 반복문임. */
            for(Edge edge : graph.get(now)) {

                /* 순회를 돌면서 거리를 기록해두는 배열에 저장된 값이 현재 비용과 간선을 타고 가는 비용을 더한 값보다 크다면 */
